...
export const ReturnRequests: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-black/30 backdrop-blur-md rounded-2xl shadow-2xl border border-purple-500/20 p-6">
        ...
        {requests.length === 0 ? (
          <div className="text-center py-12">
            <RotateCcw className="h-16 w-16 text-purple-300/50 mx-auto mb-4" />
            <h3 className="text-lg font-medium text-white mb-2">No pending return requests</h3>
            <p className="text-purple-300">All book return requests have been processed.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {requests.map((request) => (
              <div key={request.id} className="bg-black/20 border border-purple-500/30 rounded-xl p-6 hover:bg-black/30 transition-all duration-200">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center space-x-4 mb-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          <p className="font-semibold text-white">{request.user.full_name}</p>
                          <p className="text-sm text-purple-300">ID: {request.user.student_id} • Class: {request.user.class}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-2 mb-4">
                      ...
                      <div>
                        <p className="font-semibold text-white">{request.book.title}</p>
                        <p className="text-sm text-purple-300">by {request.book.author}</p>
                        {request.unique_book_id && (
                          <p className="text-xs text-gray-400 font-mono">Book ID: {request.unique_book_id}</p>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      ...
                      className="flex items-center space-x-2 bg-red-500/20 hover:bg-red-500/30 text-red-300 hover:text-red-200 px-4 py-2 rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed border border-red-500/30"
                    >
                      <X className="h-4 w-4" />
                      <span>Reject</span>
                    </button>
                    <button
                      ...
                      disabled={processingRequest === request.id}
                      className="flex items-center space-x-2 bg-green-500/20 hover:bg-green-500/30 text-green-300 hover:text-green-200 px-4 py-2 rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed border border-green-500/30"
                    >
